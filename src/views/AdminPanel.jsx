...
const AdminPanel = ({ 
  ...
  // Filtre d'activité pour les métriques
  ...
  // ============================================
  // NOUVEAU: Calcul des 20 dernières réponses de TOUTES les joueuses
  // ============================================
  const last20Responses = useMemo(() => {
    const allResponses = [];
    ...
    // Trier par date décroissante et prendre les 20 premières
    allResponses.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    return allResponses.slice(0, 20);
  }, [players]);

  // Calcul des blessures uniques
            ...
            <>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Les 20 dernières réponses de toutes les joueuses, par ordre chronologique décroissant
                </p>

                {last20Responses.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                  ...
                  <div className="space-y-3">
                    {last20Responses.map((response) => {
                      ...
                      return (
                        <div 
                          key={response.id}
                          className="flex items-center justify-between p-4 border-2 border-gray-200 rounded-lg hover:border-blue-300 hover:bg-blue-50 transition-all cursor-pointer"
                          onClick={() => setSelectedResponseDetail(response)}
                        >
                          <div className="flex-1">
                            <div className="flex items-center space-x-3 mb-2">
                              <span className={`px-3 py-1 rounded-full text-xs font-medium ${typeColors[response.type]}`}>
                                {typeLabels[response.type]}
                              </span>
                              {response.data?.activite && (
                                <span className="px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                  {activiteLabels[response.data.activite] || response.data.activite}
                                </span>
                              )}
                              <span className="font-semibold text-gray-900">{response.playerName}</span>
                            </div>
                            ...
                            <div className="text-sm text-gray-700 space-y-1">
                              {response.type === 'pre' && (
                                <>
                                  {response.data?.motivation && <p>Motivation: {response.data.motivation}/20</p>}
                                  {response.data?.fatigue && <p>Fatigue: {response.data.fatigue}/20</p>}
                                  {response.data?.plaisir && <p>Plaisir: {response.data.plaisir}/20</p>}
                                  {response.data?.cycle_phase && response.data.cycle_phase !== '' && (
                              ...
                              {response.type === 'post' && (
                                <>
                                  {response.data?.intensite_rpe && <p>RPE: {response.data.intensite_rpe}/20</p>}
                                  {response.data?.plaisir_seance && <p>Plaisir: {response.data.plaisir_seance}/20</p>}
                                </>
                              )}
                              {response.type === 'match' && response.data?.intensite_rpe && (
                                <p>RPE: {response.data.intensite_rpe}/20</p>
                              )}
                              {(response.data?.injuries && response.data.injuries.length > 0) && (
                                <p className="text-red-600 font-medium">⚠️ {response.data.injuries.length} blessure(s) signalée(s)</p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="text-right ml-4">
                            ...
                            <button className="mt-2 px-3 py-1 bg-blue-500 text-white text-xs rounded hover:bg-blue-600 transition-all">
                              👁️ Voir détail
                            </button>
                          </div>
                        </div>
                  ...
                  <div className="flex items-center space-x-3 flex-wrap">
                    ...
                    <span className="px-3 py-1 bg-white/20 rounded-full text-sm">
                      📝 {(() => {
                        ...
                        return typeLabels[selectedResponseDetail.type] || selectedResponseDetail.type;
                      })()}
                    </span>
                    <span className="px-3 py-1 bg-white/20 rounded-full text-sm">
